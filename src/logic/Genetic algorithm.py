...
def select_parents(population, scores, parents_number):
    ...
    while(not have_parents):
        max = 0

        for i in range(0, len(scores_copy)):
            if(purpose == "Maximize"):
                if(scores_copy[i] > scores_copy[max]):
                    max = i
            elif(purpose == "Minimize"):
                if(scores_copy[i] < scores_copy[max]):
                    max = i
        if len(parents) != parents_number:
            parents.append(population_copy[max])
            del population_copy[max]
            del scores_copy[max]
        else:
            have_parents = True
    return parents
...
def breed_two_parents(parent_1, parent_2, childs, breed_reg):
    ...
    for i in range(len(combinations)):
        ...
        if (len(childs) != population_size) and variables_are_in_range(individual):

            if print_trace_bool or write_trace_bool:
                breed_reg.append(
                    "Child: " + individual_to_string(individual))
